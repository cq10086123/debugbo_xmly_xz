...
- **设备绑定（一卡一机）**：见下方「进阶能力」第 5 节，默认关闭，后台一键开启。
...
防止一张卡密多人共用：卡密按**席位**绑定设备，新设备登录自动顶掉旧设备（谁最后登录谁用）。

**席位模型**：每张卡密有 `web`（网页）与 `extension`（插件）两个席位，各允许 `max_devices` 台设备（默认 1，生成卡密/编辑弹窗可调 1~10）。同一台电脑正常使用 = 网页占网页席 + 插件占插件席，互不干扰；他人用同一卡密在别的电脑登录会顶掉对应席位的旧设备。

**行为**：
- 登录绑定：首次登录把客户端设备 ID（网页 localStorage / 插件 chrome.storage，UUID 持久化）绑到对应席位。
- 顶号换绑：席位已满时新设备登录 → 淘汰最早绑定的设备，其全部会话立即失效；旧设备下次请求得到 401「账号已在其他设备登录」。网页 60s 心跳、插件 1 分钟轮询，分钟级感知被踢。
- 防拷贝：token 必须与设备 ID 匹配（`X-Device-Id` 头 / 下载直链 `&device=` 参数），拷贝 token 到别的浏览器 → 401「登录环境校验失败」。
- 换机/售后：管理后台「卡密管理 → 设备」可查看绑定详情（设备尾号/绑定时间/最后活跃/最近 IP），支持解绑单台、解绑全部、踢下线；用户清缓存换电脑则自动换绑，无需找管理员。
...
| `token_multi_ip_kick` | 同 token 10 分钟内多公网 IP 并用 → 强制下线（防共享） | 开 |
| `device_seat_ip_mismatch` | 网页席与插件席出口 IP 不一致处置：`alert` 记告警日志 / `enforce` | alert |

**平滑过渡**：开关关闭时登录不要求设备 ID、鉴权跳过设备校验；升级部署时已登录的存量会话不受影响，下次登录起才写入绑定。开启后未携带设备 ID 的旧版客户端登录会收到 403 提示升级（网页刷新即更新；插件需更新到 0.6.0+）。
