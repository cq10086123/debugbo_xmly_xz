...
- **设备绑定（宽松按IP / 严格按设备）**：见下方「进阶能力」第 5 节，默认关闭，后台一键开启。
...
### 5. 设备绑定（顶号换绑，支持宽松/严格两种粒度）

防止一张卡密多人共用：卡密按**席位**绑定登录环境，新环境登录自动顶掉旧环境（谁最后登录谁用）。

**绑定粒度**（`device_binding_scope`，可在后台切换）：
- **`ip` 按网络 IP（宽松，默认）**：同一出口 IP 下**不限设备与浏览器数量** —— 同一家庭宽带里 Chrome / Edge / 手机 / 插件随便用，互不顶号；只有换到**另一个公网 IP**（另一个家庭/网络）登录才算换设备，触发顶号换绑。IP 做网段归一化（IPv4 按 /24、IPv6 按 /48、局域网/内网统一视为同一网络），家庭宽带重拨的小幅跳变、IPv6 后缀轮换不会误踢。
- **`device` 按浏览器设备（严格）**：每席位绑定具体浏览器的持久化设备 ID，同一台电脑的不同浏览器也算不同设备（历史行为）。

**席位模型**：每张卡密有 `web`（网页）与 `extension`（插件）两个席位，各允许 `max_devices` 个绑定（默认 1，生成卡密/编辑弹窗可调 1~10；`ip` 粒度下即「允许的网络数」，设 2 可同时容纳家里+公司两个网络）。

**行为**：
- 登录绑定：`ip` 粒度绑「出口网络键」（如 `1.2.3.0/24`）；`device` 粒度绑客户端设备 ID（网页 localStorage / 插件 chrome.storage，UUID 持久化）。
- 顶号换绑：席位已满时新环境登录 → 淘汰最早的绑定，其全部会话立即失效；旧环境下次请求得到 401「账号已在其他网络/设备登录」。网页 60s 心跳、插件 1 分钟轮询，分钟级感知被踢。
- 防拷贝：`ip` 粒度下 token 拿到别的网络使用 → 401「网络环境已变更」；`device` 粒度下 token 必须与设备 ID 匹配（`X-Device-Id` 头 / 下载直链 `&device=` 参数）。
- 换机/售后：管理后台「卡密管理 → 设备」可查看绑定详情（设备尾号或网络段/绑定时间/最后活跃/最近 IP），支持解绑单条、解绑全部、踢下线；用户换网络/换电脑重新登录即自动换绑，无需找管理员。

**配置项**（后台「系统配置」，即时生效）：

| 配置键 | 说明 | 默认 |
|--------|------|------|
| `device_binding_enabled` | 设备绑定总开关；关闭 = 完全恢复历史行为（一键回退） | 关 |
| `device_binding_scope` | 绑定粒度：`ip` 按网络（宽松，同 IP 不限设备）/ `device` 按浏览器（严格） | ip |
| `trust_proxy_header` | 信任反向代理传递的 `X-Forwarded-For` / `X-Real-IP`（**部署在 nginx 等可信反代后必须开启**，否则所有用户 IP 都是代理 IP，按网络绑定会失效；直连部署务必保持关闭防伪造） | 关 |
| `session_idle_days` | 会话空闲过期天数 | 7 |
| `token_multi_ip_kick` | 同 token 10 分钟内多公网网段并用 → 强制下线（防共享） | 开 |
| `device_seat_ip_mismatch` | 网页席与插件席出口 IP 不一致处置：`alert` 记告警日志 / `enforce`（`ip` 粒度下自动跳过） | alert |

**平滑过渡**：开关关闭时登录不要求设备 ID、鉴权跳过设备校验；升级部署时已登录的存量会话不受影响，下次登录起才写入绑定。**切换绑定粒度不会踢出已在线用户**（另一粒度产生的会话直接放行），下次登录起按新粒度绑定。`ip` 粒度下旧版客户端（不带设备 ID）也能正常登录；`device` 粒度下未携带设备 ID 会收到 403 提示升级（网页刷新即更新；插件需更新到 0.6.0+）。
