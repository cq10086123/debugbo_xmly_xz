...
- 每张卡允许 `max_devices` 个网络（默认 1，生成卡密/编辑弹窗可调 1~10，设 2 可同时容纳家里+公司两个网络）。
- 顶号换绑：名额已满时在**另一个公网 IP** 登录 → 淘汰最早绑定的网络，其全部会话立即失效；旧网络下次请求得到 401「已在其他网络登录」。网页 60s 心跳、插件 1 分钟轮询，分钟级感知被踢。
- 防外带：token 拿到别的网络使用 → 401「网络环境已变更」（仅拒绝不顶号，回原网络自动恢复；只有在新网络"登录"才触发换绑）。
