# ═══════════════════════════════════════════════════════════════
# 【音频】听友FM (tingyou8) —— 整段复制粘贴到「音频」tab 即可
# 引擎契约（与 C:\Users\11754\Desktop\1\py 一致）：def parse(params)
#   params 含: bookId / chapterId / trackId / rid 及章节结果透传字段
#   返回: 音频直链字符串（http/https 开头）  或  {"url":..., "headers":{...}}
#
# ⚠️ 下方 BASE + "/track/url" 的接口路径为占位推测，
...
def parse(params):
    """获取单集音频直链（引擎契约：def parse(params)，返回 url 字符串 或 {url, headers}）"""
    book_id = params.get("bookId", "")
    chapter_id = params.get("chapterId", "") or params.get("trackId", "")
    base_url = f"{BASE}/track/url"   # TODO: 路径为推测，需抓包核实（搜索为 /apk/search）
    try:
        ...
        body = json.dumps({"albumId": book_id, "trackId": chapter_id},
    ...
    # 真实字段名未确认，先兼容几种常见结构
    url = (data.get("url") or data.get("play_url") or data.get("src") or "")
    return url
