...
#
# 【封面字段】★注意★：封面必须映射到 bookImage（或 cover），例如
#     book['bookImage'] = item.get('上游任意字段名')
#   系统只读这两个标准字段，不会去猜其它键。原因：搜索结果里的自定义字段
#   会原样透传给章节脚本，其中常混有主播头像(avatar)、站点标识(logo)、
#   二维码、分享图等图片链接；若自动猜测，很容易把头像当成封面显示。
#   映射后，网页搜索页与 AI 助手都能显示封面（书名重复时用来辨认是哪一本）。
#   保存后可在后台「测试」接口，结果会显示封面是否读取成功；
#   若未读到，还会提示当前结果里哪些字段疑似封面，供你选择映射。
#
