  ...
  <div class="guide">
    ...
    <section class="card-panel pad">
      ...
      <dl class="faq">
        ...
        <dt>每一集都弹出「另存为」窗口，必须手动点保存？（常见于 Mac）</dt>
        <dd>这不是插件或网页的问题，而是 Chrome 浏览器自己的设置：<b>「下载前询问每个文件的保存位置」</b>被开启了——Mac 版 Chrome 较常默认开启，Windows 默认关闭，所以不同电脑表现不一样，插件无法替你修改这个设置。打开 <code>chrome://settings/downloads</code>，关闭该开关，之后每一集都会自动保存到「下载」文件夹。另：如果 Mac 的下载位置设在了 iCloud 云盘等同步目录，建议改回本机「下载」文件夹，避免文件隐藏或未同步的问题。</dd>
        <dt>文件管理里集数太多？</dt>
