# ═══════════════════════════════════════════════════════════════
# 【搜索】听友FM (tingyou8) —— 整段复制粘贴到「搜索」tab 即可
# 引擎契约（与 C:\Users\11754\Desktop\1\py 一致）：def parse(params)
#   params 含: keyword / encoded_keyword / timestamp / timestamp_sec / page
#   返回: list[dict]，每项必含 id、bookTitle（可选 bookImage/bookAnchor/count 等）
# ═══════════════════════════════════════════════════════════════
import requests
import json
import secrets
...
# ── dfp 纯算法生成（libtokendecoder.so encryptFingerprint 复现）──
_MASK = 0xFFFFFFFF
...
def parse(params):
    """搜索书籍（引擎契约：def parse(params)，返回 list[{id, bookTitle, ...}]）"""
    keyword = params.get("keyword", "")
    page = int(params.get("page", 1))
    base_url = "https://azybk.tingyou8.vip/apk/search"
    try:
        ...
        body = json.dumps({"keyword": keyword, "page": page},
                          ensure_ascii=False, separators=(",", ":")).encode("utf-8")
        response = requests.post(base_url, data=_encrypt(body).hex(),
                                 headers=headers, timeout=15, verify=False)
        if response.status_code in (401, 403):
            cred = _get_credentials(force=True)
            headers["Authorization"] = "Bearer " + cred["token"]
            headers["Cookie"] = cred.get("dfp", _gen_dfp()) + "; session=" + cred.get("session", "")
            response = requests.post(base_url, data=_encrypt(body).hex(),
                                     headers=headers, timeout=15, verify=False)
    ...
    for item in data.get("results", []):
        result.append({
            "id": str(item.get("id", "")),          # 必填：作为 book_id
            "bookTitle": item.get("title", ""),      # 必填：书名
            "bookImage": item.get("cover_url", ""),  # 封面
            "bookAnchor": item.get("author", "") or item.get("teller", ""),  # 主播
            "count": item.get("count", 0),           # 集数
            # "albumId": item.get("id"),             # 如需自定义字段透传，可加
        })
